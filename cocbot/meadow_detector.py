...
class MeadowParallelogramDetector:
    ...
    def detect(self, screen_bgr, min_area_ratio=0.07):
        """返回 (center, box_points, mask)。"""
    ...
    def _draw_detect_collage(self, original, mask, line_img, center, box, vertices, debug_text):
        mask_bgr = cv2.cvtColor(mask, cv2.COLOR_GRAY2BGR)
        overlay = cv2.addWeighted(original, 0.75, mask_bgr, 0.25, 0)

        if center is not None:
            cv2.drawMarker(overlay, (int(center[0]), int(center[1])), (0, 0, 255), cv2.MARKER_TILTED_CROSS, 50, 3)

        if box is not None and len(box) >= 4:
            cv2.polylines(overlay, [np.array(box, dtype=np.int32)], True, (0, 255, 0), 2)

        for v in vertices:
            cv2.circle(line_img, (int(v[0]), int(v[1])), 6, (0, 0, 255), -1)

        for title, panel in [
            ("Original", original),
            ("Mask", mask_bgr),
            ("Lines + Intersections", line_img),
            ("Overlay + Center", overlay),
        ]:
            cv2.rectangle(panel, (0, 0), (panel.shape[1], 38), (15, 15, 15), -1)
            cv2.putText(panel, title, (12, 26), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 2, cv2.LINE_AA)

        top = np.hstack([original, mask_bgr])
        bottom = np.hstack([line_img, overlay])
        canvas = np.vstack([top, bottom])

        cv2.rectangle(canvas, (0, canvas.shape[0] - 40), (canvas.shape[1], canvas.shape[0]), (20, 20, 20), -1)
        cv2.putText(canvas, debug_text, (10, canvas.shape[0] - 12), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2, cv2.LINE_AA)
        return canvas
    ...
    def detect_with_debug(self, screen_bgr, min_area_ratio=0.07, debug_output=False, debug_dir=None, debug_prefix="meadow_detect"):
        ...
        line_img = screen_bgr.copy()

        moments = cv2.moments(mask)
        if moments["m00"] == 0:
            if debug_output and debug_dir:
                os.makedirs(debug_dir, exist_ok=True)
                canvas = self._draw_detect_collage(screen_bgr.copy(), mask, line_img, None, None, [], "m00=0, no center")
                cv2.imwrite(os.path.join(debug_dir, f"{debug_prefix}_m00_zero.png"), canvas)
        ...
        for theta in self.angles:
            ...
            for rho in rhos:
                pts = self.get_line_points(theta, rho, w, h)
                if not pts:
                    continue
                color = (255, 0, 0) if abs(theta - self.angles[0]) < 1e-6 else (0, 255, 255)
                cv2.line(line_img, pts[0], pts[1], color, 2)
        ...
        if len(vertices) >= 4:
            ...
            if len(pts) >= 4:
                ...
                if len(hull) >= 4:
                    # 直接使用交点构成的凸包作为最终多边形，保证与参与计算的直线方向一致。
                    box = hull.reshape(-1, 2).astype(int)
                    center = (int(np.mean(box[:, 0])), int(np.mean(box[:, 1])))
                    if debug_output and debug_dir:
                        os.makedirs(debug_dir, exist_ok=True)
                        debug_text = (
                            f"center={center}, hsv=[{self.hsv_lower.tolist()}->{self.hsv_upper.tolist()}], "
                            f"search_step={self.search_step}, min_line_len={self.min_line_len}"
                        )
                        canvas = self._draw_detect_collage(
                            screen_bgr.copy(),
                            mask,
                            line_img,
                            center,
                        ...
                        cv2.imwrite(os.path.join(debug_dir, f"{debug_prefix}_ok.png"), canvas)
        ...
        if best is not None:
            ...
            if debug_output and debug_dir:
                os.makedirs(debug_dir, exist_ok=True)
                debug_text = (
                    f"fallback contour, center={center}, hsv=[{self.hsv_lower.tolist()}->{self.hsv_upper.tolist()}], "
                    f"search_step={self.search_step}, min_line_len={self.min_line_len}"
                )
                canvas = self._draw_detect_collage(screen_bgr.copy(), mask, line_img, center, box, vertices, debug_text)
                cv2.imwrite(os.path.join(debug_dir, f"{debug_prefix}_fallback.png"), canvas)
            return center, box, mask

        if debug_output and debug_dir:
            os.makedirs(debug_dir, exist_ok=True)
            debug_text = "no contour found for fallback"
            canvas = self._draw_detect_collage(screen_bgr.copy(), mask, line_img, (cx_pre, cy_pre), None, vertices, debug_text)
            cv2.imwrite(os.path.join(debug_dir, f"{debug_prefix}_no_contour.png"), canvas)
